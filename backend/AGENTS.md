...
- Domain types live alongside their logic: room types/messages in `src/room.gleam`, registry messages in `src/room_registry.gleam`.
- Connection/session state and REST contexts live in `src/domain/session.gleam`.
- `src/domain/request.gleam` and `src/domain/response.gleam` define transport-agnostic messages.
- `src/handlers/` contains transport-agnostic logic grouped by feature (rooms, chat, debug, session) with a shared dispatcher.
- `src/transport/incoming.gleam` and `src/transport/outgoing.gleam` are wire-only JSON decode/encode modules.
- Shared message types live in `src/chat.gleam` and `src/protocol.gleam` (client/server wire protocol).
- `test/` holds Gleeunit tests (e.g., `test/backend_test.gleam`).
- Handler logic tests live under `test/handlers/`.
...
- Update `AGENTS.md` whenever architecture, key commands, or module ownership changes.
...
- The client (connection process) builds a `ChatMessage` that includes:
  ...
  - `kind` (text/image/…)
  - `payload` (content)
  - optional metadata (timestamp, message id)
